...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const c = await AuditService.openAudit(id, session.user.id);
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (url.searchParams.get('action') === 'reject') {
      ...
      const c = await AuditService.reject(id, session.user.id, reason);
      const full = await prisma.case.findUnique({ where: { id: id } });
      if (full) {
      ...
      const c = await AuditService.sign(id, session.user.id, body.signatureUrl, body.comment);
      const full = await prisma.case.findUnique({ where: { id: id } });
      if (full) {
        await NotificationService.create(full.registeredById, 'AUDIT_RESULT', {
          refType: 'Case',
