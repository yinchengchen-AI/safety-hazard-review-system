...
| Review            | id, caseId, reviewerId, status(in_progress/submitted/returned), startedAt, submittedAt, conclusion(通过/不通过/部分通过), summary, score, **claimedById, claimedAt, lastActiveAt**                       |
...
**正式状态转移表（from × event → to）：**

| 当前状态            | 事件                | 下一状态 | 触发者    | 副作用                                                        |
| ------------------- | ------------------- | -------- | --------- | ------------------------------------------------------------- |
| (前端草稿)          | submit_registration | 待复核   | 监管员    | 创建 Case + 初始 Review(空)                                   |
| 待复核              | submit_review       | 待审核   | 监管员    | 当前 Review.status=submitted，Case.status=待审核              |
| 待审核              | open_audit          | 审核中   | 科长      | Case.lockedById/lockedAt，AuditLog 记录"领取"                 |
| 审核中              | sign                | 已销案   | 科长      | 写 AuditSignature，Case.closedAt，解除锁定                    |
| 审核中              | reject              | 待复核   | 科长      | 原 Review.status=returned，新建空 Review 等待新一轮；解除锁定 |
| Review(in_progress) | take_over           | (不变)   | 监管员    | Review.claimedById 切换到接管者（详见 §5.3）                  |
| 审核中              | reclaim_idle        | 待审核   | 系统 cron | Case.lockedById=null（详见 §5.7）                             |

非法转移（如"待复核 → 已销案"）必须抛 `BusinessError(code=invalid_transition)`，由 `CaseService.transitionStatus` 统一校验；`Review.claimedById` 的转移由 `ReviewService.takeOver` 单独处理。
...
监管员在 `/cases/[id]/review` 点"开始复核" → 服务端用 `SELECT ... FOR UPDATE` 锁定 Review 行 → 写入 `claimedById` + `claimedAt` + `lastActiveAt = now()`；其他监管员进入只读模式，提示"已被 X 监管员领取"。

**接管机制**：其他监管员可点"接管" → 校验 `Review.lastActiveAt`（24h 内禁止接管，避免覆盖活跃草稿）→ 切换 `claimedById` → AuditLog 记录"接管"。原草稿（ReviewItemResult、ReviewPhoto）保留。

填清单（按 Case.hazardTypeId 加载 ChecklistTemplate）→ 逐项 pass/fail/N/A + 备注 + 现场拍照（每次保存更新 `lastActiveAt`）→ 写总体结论（通过/不通过/部分通过） + 说明 → 提交 Review → Case 状态 = 待审核。
...
- 监管员提交 Review → 给**所有科长**发"待审核通知"（v0.1 单局，科长规模小；后续多局/多科室时再按 scope 路由），各科长工作台"待审核"计数 +1
- 科长通过/驳回 → 给监管员发"审核结果通知"

### 5.6 离线同步流程

- **离线时**：Service Worker 缓存已访问的案件 / 清单模板；照片 → IndexedDB blob；复核草稿 → IndexedDB；待提交操作 → IndexedDB 操作队列（每条带 `clientId` UUID 用于幂等）
- **联网时**：`window.online` 事件触发 → sync worker 顺序拉取 IndexedDB pending 项 → POST `/api/sync`（带 clientId）→ 后端 upsert 到 `OfflineSyncQueue`（用 clientId 幂等去重）→ 创建/更新 Review + 写 AuditLog → 返回结果 → 客户端根据结果删除本地副本
- **失败**：指数退避重试 3 次 → 标 failed → 推送通知给监管员 → 「我的同步」页手动重试
- **冲突**：以"最后提交成功者"为准；离线提交发现 Case.status = 已销案 → 提示"该案件已结案，无需复核"并放弃

**客户端 vs 服务端队列边界**：

- IndexedDB = **客户端**离线缓冲（断网期间所有写入都进这里：照片 blob / 草稿 / 操作队列）
- Postgres `OfflineSyncQueue` = **服务端**处理队列（联网后客户端用 clientId 推上来；服务端做幂等去重 + cron 重试兜底 + 审计追踪）

### 5.7 资源回收 cron

进程内 cron 每天 02:00（与 §5.5 通知 cron 错开）扫描：

- `Review.status = in_progress AND lastActiveAt < now() - 24h` → 释放 `claimedById/claimedAt = null`，status 保持 in_progress；AuditLog 记录"复核超时释放"
- `Case.status = 审核中 AND lockedAt < now() - 24h` → 释放 `lockedById/lockedAt = null`，Case 回到 `待审核`；AuditLog 记录"审核超时释放"

此 cron 与 §5.5 的"超时提醒"通知互补：提醒推给人看（可能还在路上），cron 给数据松绑（避免长期锁死导致资源无法流转）。
